...
export default function SecuritySettingsCard() {
  const { showNotification } = useDoctor();
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      setConfirmPassword("");
      if (showNotification) {
        showNotification("Password changed successfully! Security settings updated.");
      }
    }, 1000);
  };
  ...
  return (
    <div className="bg-white border border-gray-150 rounded-2xl p-5 shadow-sm space-y-4">
      <div className="flex items-center gap-2 pb-2 border-b border-gray-100">
        ...
        <h4 className="text-sm font-bold text-gray-900 uppercase tracking-wider">Security & Account Settings</h4>
      </div>

      <form onSubmit={handleSubmit} className="space-y-4 text-sm">
        {error && (
          <div className="p-3 bg-danger/10 text-danger rounded-xl text-xs font-semibold leading-normal">
            {error}
          </div>
        )}

        {success && (
          <div className="p-3 bg-success/10 text-success rounded-xl text-xs font-semibold flex items-center gap-2 leading-normal">
